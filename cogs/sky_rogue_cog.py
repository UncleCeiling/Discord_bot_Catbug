...
class SkyRogue(commands.Cog):
    ...
    @app_commands.describe(
        ...
        modded="Choose whether to include 'Modded' equipment in the loadout.",
    ...
    async def skyrogue(
        ...
        col2, col3, col4 = (
            len(max(loadout.weapon_codes(), key=len)),
            len(max(loadout.weapon_names(), key=len)),
            len(max(loadout.weapon_types(), key=len)),
        )
